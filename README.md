...
При `--clip` пароль не печатается в stdout — он попадает прямо в системный буфер обмена (`pbcopy` / `wl-copy` / `xclip` / `xsel`) и автоматически вытирается через `--clip-ttl`. Процесс блокируется до истечения TTL; при прерывании (Ctrl+C, SIGTERM) буфер очищается до выхода.

> **Внимание:** менеджеры истории буфера обмена (Maccy, CopyQ и т.п.) могут сохранить пароль в своей истории даже после очистки — jtgpwgen не может на это повлиять. Если используете такой менеджер, добавьте jtgpwgen-пароли в исключения или очищайте историю вручную.
